...
SquareIsFree <- function(chess.board, n.queens, row.i) {
  return(chess.board$column[row.i] &
         chess.board$diagonal.up[n.queens + chess.board$column.j + 1 - row.i] &
         chess.board$diagonal.down[chess.board$column.j + row.i])
}
...
RemoveQueen <- function(chess.board, n.queens, row.i) {
  ...
  chess.board$column.j <- chess.board$column.j - 1
  chess.board$diagonal.down[chess.board$column.j + row.i] <- TRUE
  return(chess.board)
}
